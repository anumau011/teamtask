...
export default function RegisterPage() {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await api.post('/auth/register', { ...values, role: 'developer' });
      login(response.data);
      navigate('/dashboard', { replace: true });
    } catch (error) {
      const responseErrors = error?.response?.data?.errors;
      if (Array.isArray(responseErrors)) {
        setServerError(responseErrors.map((entry) => entry.message).join(', '));
        return;
      }
  ...
  return (
    <AuthShell title="Create account" subtitle="Start managing work">
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        <div>
          ...
          />
          {errors.name ? <p className="mt-2 flex items-center gap-1 text-sm text-red-600"><AlertCircle className="h-3 w-3" />{errors.name.message}</p> : null}
        </div>
        <div>
          <label className="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-700">
